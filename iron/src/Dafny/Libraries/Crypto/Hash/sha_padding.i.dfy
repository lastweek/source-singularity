    ...
    ensures Mod32_const(message_len + NumPaddingZeroes(message_len)) == 31;
{
    calc
    {
        Mod32_const(message_len + NumPaddingZeroes(message_len));
        { reveal_NumPaddingZeroes(); reveal_PaddedLength(); }
        Mod32_const(message_len + (959 - (message_len % 512)) % 512);
        { reveal_Mod32_const(); }
        31;
    }
}
    ...
    modifies a;
{
    ...
    calc {
        b + numPad + 33;
        < { reveal_PaddedLength(); } PaddedLength(b);
        <= a.Length * 32;
    }

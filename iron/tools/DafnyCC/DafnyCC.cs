...
public class DafnyCC: DafnySpec
{
    ...
    public static new void Main(string[] args)
    {
        try
        {
            ...
            List<string> files = Bpl.CommandLineOptions.Clo.Files;
            if (files.Count == 0)
            {
    ...
    public override void AddMethodAsLemma(Method method)
    {
        if (lazyLemmas.Contains(method.Name))
        {
            ...
            lemmas.Add(new LemmaCall("Seq", (Type)null,
                "call " + ProcName(true, SimpleName(method.FullSanitizedName)) + "();",
...
*/
        ...
        Dictionary<string,TypeParameter> substArgs)
    {
        ...
        string name = FunName(SimpleName(function.FullSanitizedName));
        ...
        if (hidden)
        {
            revealCall = new CallStmt(tok, tok, new List<Expression>(), new ThisExpr(tok),
                "reveal_" + function.Name, new List<Expression>());
            revealCall.IsGhost = true;
            revealCall.TypeArgumentSubstitutions = new Dictionary<TypeParameter,Type>();
            revealCall.Method = FindMethod(revealCall.MethodName);
    ...
    public Tuple<Method,TypeApply> GetSeqBuildMethod(Type t, SeqTree tree, List<bool> elemDimensions)
    {
        ...
        Func<string,Type,Expression> idExpr = (x, typ) => {
            ...
            e.Type = typ;
            e.Var = new VarDecl(tok, tok, x, typ, false);
            return e;
        };
        ...
        for (int i = elemDimensions.Count; i > 0;)
        {
            ...
            if (elemDimensions[i])
            {
                ...
                else
                {
                    ...
                    CallStmt callStmt = new CallStmt(tok, tok,
                        ...
                        new ThisExpr(tok), "seq_Append", new List<Expression>
                            { idExpr("s" + i, dataType), idExpr("s", dataType) });
                    var seqAppend = GetSeqMethod(t, "seq_Append");
                    callStmt.Method = seqAppend.Item1;
                    callStmt.TypeArgumentSubstitutions = seqAppend.Item2.typeArgs;
                    stmts.Add(callStmt);
                }
            }
            else
            {
                ...
                CallStmt callStmt = new CallStmt(tok, tok,
                    new List<Expression> {},
                    new ThisExpr(tok), "lemma_Seq_Cons", new List<Expression>
                        { idExpr("a" + i, elemType), idExpr("s", dataType) });
                var lemmaCons = GetSeqMethod(t, "lemma_Seq_Cons");
                callStmt.Method = lemmaCons.Item1;
                callStmt.TypeArgumentSubstitutions = lemmaCons.Item2.typeArgs;
        ...
        Attributes attrs = new Attributes("dafnycc_conservative_seq_triggers", new List<Attributes.Argument>(), null);
    ...
    public Dictionary<string,string> CompileCodeStart(DafnyCC_Options options, List<string> files)
    {
    ...
    public void CompileCode(DafnyCC_Options options, List<string> files)
    {

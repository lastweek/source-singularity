...
public class CompileMethod: CompileMethodGhost
{
    ...
    void Jump(string label, Expression exp, bool jumpIf = true)
    {
        ...
        else if (binary != null && binary.Op == BinaryExpr.Opcode.Iff && jumpIf)
        {
            Jump(label, DafnySpec.MakeBinaryExpr(BinaryExpr.Opcode.Or, BinaryExpr.ResolvedOpcode.Or, Type.Bool,
                ...
                DafnySpec.MakeBinaryExpr(BinaryExpr.Opcode.And, BinaryExpr.ResolvedOpcode.And, Type.Bool,
                    new UnaryExpr(binary.tok, UnaryExpr.Opcode.Not, binary.E0),
                    new UnaryExpr(binary.tok, UnaryExpr.Opcode.Not, binary.E1))), jumpIf);
        }
        else if (binary != null && binary.Op == BinaryExpr.Opcode.Imp)
        {
            Jump(label, DafnySpec.MakeBinaryExpr(BinaryExpr.Opcode.Or, BinaryExpr.ResolvedOpcode.Or, Type.Bool,
                new UnaryExpr(binary.tok, UnaryExpr.Opcode.Not, binary.E0), binary.E1), jumpIf);
        }
        else if (binary != null && (
        ...
        else if (fieldSelect != null && fieldSelect.FieldName.EndsWith("?"))
        {
            ...
            RtlVar e0 = AsVarOrTemp(fieldSelect.Obj);
            JumpIfHasType(label, e0, constructor, fieldSelect.Obj.Type, jumpIf);
        }
        else
        {
    ...
    void AddExpression(RtlVar dest, Expression exp)
    {
                ...
                switch (binary.Op)
                {
                    ...
                    case BinaryExpr.Opcode.Mod:
                    {
                        ...
                        AddCallInstruction(
                            ...
                            m.Ins, SimpleName(m.FullSanitizedName),
        ...
        {
            RtlVar eArr = AsVarOrTemp(fieldSelect.Obj);
            ...
            && fieldSelect.Field is DatatypeDestructor)
        {
            ...
            if (minVerify)
            {
                ...
                JumpIfHasType(ok, e0, field.EnclosingCtor.Name, fieldSelect.Obj.Type, true);
                Label(fail, true);
                Jump(fail);
                Label(ok, false);
            }
            AddFieldSelect(dest, e0, fieldSelect.Obj.Type, fieldSelect.FieldName, exp.Type, GhostExpression(exp));
        }
    ...
    List<Tuple<string,string>> InstructionAttributes(Attributes attrs)
    {
        ...
        return attrs.Args.ConvertAll(a => a.S.Contains("@")
            ? Tuple.Create(a.S.Substring(0, a.S.IndexOf('@')), a.S.Substring(a.S.IndexOf('@') + 1))
            : Tuple.Create(a.S, (string)null));
    }
    ...
    private void AddCall(List<RtlVar> destVars, bool isGhost, bool isHeap, ICallable target, List<Expression> args, List<Formal> rets)
    {
        AddCall(destVars, isGhost, isHeap, target.Ins, SimpleName(target.FullSanitizedName), args, rets);
    }

    private void AddCall(List<RtlVar> destVars, bool isGhost, bool isHeap, ICallable target,
        List<RtlVar> argVars, List<Expression> args, List<Formal> rets, TypeApply typeApply)
    {
        ...
        {
            AddCallInstruction(destVars, target.Ins, SimpleName(target.FullSanitizedName), args,
                attrs);
        }
        else
        {
    ...
    private void AddCall(List<RtlVar> destVars, bool isGhost, bool isHeap, ICallable target,
        List<Expression> args, List<Formal> rets, TypeApply typeApply)
    {
    ...
    public override void AddResolvedGhostStatement(Statement stmt)
    {
        ...
        if (matchStmt != null)
        {
            ...
            foreach (MatchCaseStmt c in cases)
            {
                ...
                stmts.Add(new RtlGhostStmtComputed(s => "if (" + x + " is " + c.Ctor.Name + ") {",
                    new RtlExp[] {}));
                Indent();
                c.Body.ForEach(AddGhostStatement);
                Unindent();
        ...
        {
            base.AddResolvedGhostStatement(stmt);
        }
    }
    ...
    void AddResolvedStatement(Statement stmt)
    {
        ...
        {
            AddVarDecl(varDecl.Name, varDecl.Type, varDecl.IsGhost);
        }
        ...
        {
            AddGhostStatement(calcStmt);
        }
        else if (forallStmt != null)
        {
            AddGhostStatement(forallStmt);
        }
        else if (printStmt != null)
        {
            ...
            AddCall(new List<RtlVar>(), false, true, m,
                new List<Expression>() { printStmt.Args[0].E }, m.Outs);
        }
        else
        {
    ...
    {
        string name = GhostProcName(SimpleName(method.FullSanitizedName));

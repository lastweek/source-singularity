...
let string_of_lin (l:linearity):string =
  match (!symdiff, l) with
  | (false, Lin (LinVar, LinOur)) -> "{:linear \"our\"}"
  | (false, Lin (LinConst, LinOur)) -> "{:cnst \"our\"}"
  | (false, Lin (LinVar, LinMy)) -> "{:linear \"my\"}"
  | (false, Lin (LinConst, LinMy)) -> "{:cnst \"my\"}"
  | _ -> ""
...
let string_of_pformal ((x, t, l):pformal) = (string_of_lin l) + x + ":" + (string_of_typ t)
let string_of_formals (fs:formal list) = String.concat "," (List.map string_of_formal fs)
let string_of_pformals (fs:pformal list) = String.concat "," (List.map string_of_pformal fs)
...
let rec emit_decl (state:print_state) (l:loc, d:decl):unit =
      ...
      List.iter
        (fun (lin, c, fs) ->
          ...
          (match lin with
            ...
            | _ ->
              let sfs = string_of_pformals fs in
              let sc = (string_of_lin lin) + "#x:" + x in
              let sis = "is#" + c + "(#x);" in
              ...
              state.PrintLine ("procedure destruct##" + c + "(#me:int, #r:ReturnTo, " + sc + ") returns(" + sfs + "); requires " + sis + ens));
  ...
  | DStaticGhost (x, t, lin, _) ->
      ...
      state.PrintLine ("var " + (string_of_lin lin) + x + ":" + (string_of_typ t) + ";")
      ...
      (if !symdiff then
        ...
        (match pk with
          ...
          | Procedure _ ->
              ...
              state.PrintLine ("procedure #symdiff_proc_reqs##" + x2 + "(" + (string_of_pformals fs) + ");");
              state.PrintLine ("  ensures true " + (String.concat "" req_strings) + ";");
              symdiff_write_fun false true pres posts)
      );
      let f x pk p_fs p_rs b_opt =
        let rets = match p_rs with [] -> "" | _ -> "returns(" + (string_of_pformals p_rs) + ")" in
        let semi = match pk with Procedure _ -> ";" | Implementation -> "" in 
        state.PrintLine ((string_of_proc_kind pk) + " " + x + "(" + (string_of_pformals p_fs) + ")" + rets + semi);
        state.Indent ();
        ...
        | Some (vars, b) ->
          (
            ...
            state.Indent ();
            List.iter (fun (x, t, l) ->
              state.PrintLine ("var " + (string_of_lin l) + x + ":" + (string_of_typ t) + ";")) vars;

      ...
      | DProc (xp, _, ik, ps, rs, specs, b_opt) ->
        (
          ...
          (
            let check_spec spec =
              match spec with
              | (_, Ensures (IsRel, _)) when not !symdiff_allow ->
            ...
            let ((ghost, atomicity), _) = proc_decl in
...
let rec stmt_calls (_, stmt:stmt):string list =
  match stmt with
  | SLabel _ | SGoto _ | SReturn | SIReturn | SIfJcc _ | SAssert _ | SYield _ | SAssign _ | SExistsGhost _ -> []
...
let rec expand_inline_calls (ic:insCtxt) (InlineMap inlines) (l:loc, s:stmt):(insCtxt * loc * stmt) list list =
  ...
  | SAssert _ | SAssign _ | SIfGhost _ | SForallGhost _ | SExistsGhost _ -> []

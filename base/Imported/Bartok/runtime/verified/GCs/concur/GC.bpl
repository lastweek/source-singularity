...
function {:inline} Iso(root:[idx]int, rootAbs:[idx]obj, mem:[int][fld]int, memAbs:[obj][fld]obj, Color:[int]int, toAbs:[int]obj, allocSet:[obj]bool) returns (bool)
{
	...
	(forall x: int, f: fld :: memAddr(x) && Unalloc(Color[x]) && fieldAddr(f) ==> mem[x][f] == x) &&
...
ensures {:layer 98,100} SweepPhase(collectorPhase) && PhaseConsistent(collectorPhase, mutatorPhase);
{
  ...
  invariant {:layer 98} MsWellFormed(MarkStack, MarkStackPtr, Color, 0);
  invariant {:layer 100} memLo <= localSweepPtr && localSweepPtr <= memHi;
  invariant {:layer 99} RootScanBarrierInv(mutatorsInRootScanBarrier, rootScanBarrier);
  invariant {:layer 98,100} SweepPhase(collectorPhase) && PhaseConsistent(collectorPhase, mutatorPhase);
  invariant {:layer 100} sweepPtr == memLo;
  invariant {:layer 100} SweepInvInit(root, rootAbs, mem, memAbs, Color, toAbs, allocSet);
  invariant {:layer 100} (forall i: int :: rootAddr(i) && memAddr(root[i]) ==> Black(Color[root[i]]));
  invariant {:layer 100} (forall x: int, f: fld :: memLo <= x && x < localSweepPtr && (Unalloc(Color[x]) || White(Color[x])) && fieldAddr(f) ==> mem[x][f] == x);
  {
    call isWhite := IsWhiteByCollector(tid, localSweepPtr);
    if (isWhite) 
    {
      call YieldSweepBegin(tid, localSweepPtr, 0, true);
      fldIter := 0;
      ...
      invariant {:layer 100} (forall i: int :: rootAddr(i) && memAddr(root[i]) ==> Black(Color[root[i]]));
      invariant {:layer 100} (forall x: int, f: fld :: memLo <= x && x < localSweepPtr && (Unalloc(Color[x]) || White(Color[x])) && fieldAddr(f) ==> mem[x][f] == x);
      invariant {:layer 100} (forall f: fld :: 0 <= f && f < fldIter ==> mem[localSweepPtr][f] == localSweepPtr);
      invariant {:layer 100} White(Color[localSweepPtr]);
      {
        call InitializeFieldInSweep(tid, localSweepPtr, fldIter);
	fldIter := fldIter + 1;
        call YieldSweepBegin(tid, localSweepPtr, fldIter, true);
      }
    }
...
ensures {:atomic} |{ A:   assert mutatorTidWhole(tid) && rootAddr(x) && tidOwns(tid, x) && fieldAddr(f) && rootAddr(y) && tidOwns(tid, y) && memAddr(root[x]) && memAddrAbs(rootAbs[x]);
...
ensures {:atomic} |{A:   assert mutatorTidWhole(tid);
                         assume (memAddr(ptr) && Unalloc(Color[ptr]));
			 Color[ptr] := if sweepPtr <= ptr then BLACK() else WHITE();
	   		 toAbs[ptr] := absPtr;
			 return true;
...
procedure {:yields} {:layer 96,100} InitializeFieldInSweep({:linear "tid"} tid: Tid, ptr: int, fld: int)
ensures {:atomic} |{ A: assert tid == GcTid && memAddr(ptr) && fieldAddr(fld) && White(Color[ptr]); mem[ptr][fld] := ptr; return true; }|;
{
    yield;
    call LockAcquire(tid); // TODO: it would be better to hold the lock for longer
    call InitializeFieldInSweepLocked(tid, ptr, fld);
    call LockRelease(tid);
    yield;
}
...
ensures {:atomic} |{A: assert mutatorTidWhole(tid) && memAddr(ptr);
                         goto P, Q; 
                    ...
                    }|;
{
    ...
    if (Unalloc(color))
    {
        ...
        if (Unalloc(color))
        {
            ...
            call SetColor3(tid, ptr, color);
            call t := GetToAbs(tid);
            call SetToAbs(tid, t[ptr := absPtr]);
            call LockRelease(tid);
            yield;
            return;
        }
...
ensures {:atomic} |{ A: assert tid == GcTid; toAbs := (lambda x: int :: if memAddr(x) && White(Color[x]) then nil else toAbs[x]); return true; }|;
{
    ...
    yield;
    call LockAcquire(tid);
    call colors := GetColorsGhost(tid);
    call t := GetToAbs(tid);
    call SetToAbs(tid, (lambda x: int :: if memAddr(x) && White(colors[x]) then nil else t[x]));
    call LockRelease(tid);
    yield;
}
...
ensures {:atomic} |{ A: assert tid == GcTid && memAddr(x) && fieldAddr(f); y := mem[x][f]; return true; }|;
{
    yield;
...
ensures {:atomic} |{ A: assert mutatorTidWhole(tid) && memAddr(x) && fieldAddr(f); y := mem[x][f]; return true; }|;
{
    yield;
    call y := PrimitiveReadField(x, f);
    yield;
...
ensures {:atomic} |{ A: assert mutatorTidWhole(tid) && memAddr(x) && fieldAddr(f); mem[x][f] := y; return true; }|;
{
    yield;
...
procedure {:yields} {:layer 95,96} InitializeFieldInSweepLocked({:linear "tid"} tid: Tid, ptr: int, fld: int)
ensures {:atomic} |{ A: assert tid == GcTid && tidHasLock(tid, lock) && memAddr(ptr) && fieldAddr(fld) && White(Color[ptr]); mem[ptr][fld] := ptr; return true; }|;
{
    yield;
...
procedure {:yields} {:layer 95,96} GetColorsGhost({:linear "tid"} tid:Tid) returns(val: [int]int)
ensures {:both} |{ A: assert tidHasLock(tid, lock); val := Color; return true; }|;
{
    yield;
    call val := PrimitiveGetColorsGhost();
    yield;
}
...
procedure {:yields} {:layer 95,96} SetColor3({:linear "tid"} tid:Tid, i: int, val: int)
...
{
    yield;
    call PrimitiveSetToAbs((lambda i:int :: if memAddr(i) then nil else Int(i)));
    yield;
}

procedure {:yields} {:layer 95,96} GetToAbs({:linear "tid"} tid:Tid) returns(val: [int]obj)
ensures {:both} |{ A: assert tidHasLock(tid, lock); val := toAbs; return true; }|;
{
    yield;
    call val := PrimitiveGetToAbs();
    yield;
}

procedure {:yields} {:layer 95,96} SetToAbs({:linear "tid"} tid:Tid, val: [int]obj)
ensures {:both} |{ A: assert tidHasLock(tid, lock); toAbs := val; return true; }|;
{
    yield;
    call PrimitiveSetToAbs(val);
    yield;
}
...
procedure {:yields} {:layer 0,95} PrimitiveGetColorsGhost() returns (val: [int]int);
ensures {:atomic} |{ A: val := Color; return true; }|;
...
procedure {:yields} {:layer 0,95} PrimitiveGetToAbs() returns(val: [int]obj);
ensures {:atomic} |{ A: val := toAbs; return true; }|;

procedure {:yields} {:layer 0,95} PrimitiveSetToAbs(val: [int]obj);
ensures {:atomic} |{ A: toAbs := val; return true; }|;
